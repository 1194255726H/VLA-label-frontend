...
function normalize(item: Record<string, unknown>): VideoListItem {
  return {
    ...
    taskCurrentNode: normalizeNode(item.task_current_node), taskCurrentAssigneeId: optionalString(item.task_current_assignee_id), currentNode: normalizeNode(item.current_node), currentAssigneeId: optionalString(item.current_assignee_id), currentAssigneeName: optionalString(item.video_current_assignee_name),
