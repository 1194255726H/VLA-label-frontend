import { ArrowLeft, CircleAlert, Edit3, Eye, MoreHorizontal, Plus, Search, Tags, Trash2 } from 'lucide-react'
  ...
  return <AppShell user={session.account}>
    <section className="management-page">
      <section className="management-panel panel">
        {selected ? <>
          <header className="management-toolbar detail-toolbar"><div className="detail-title"><button className="icon-button bordered" type="button" onClick={() => setSelectedId(undefined)}><ArrowLeft size={17} /></button><div><h2>{selected.name}</h2><p>{selected.desc || '暂无标签库描述'} · {selected.count} 个启用标签</p></div></div><div><button className="secondary-button" type="button" disabled title="当前后端 API 尚未提供标签导出">导出标签</button><button className="primary-button" type="button" onClick={() => openLabel()}><Plus size={16} />创建标签</button></div></header>
          <div className="management-filters"><label><span>标签名称</span><div className="filter-control"><Search size={16} /><input value={keywordInput} onChange={(e) => setKeywordInput(e.target.value)} placeholder="请输入标签名称" /></div></label><button className="primary-button compact" type="button" onClick={() => setKeyword(keywordInput.trim())}>查询</button><button className="secondary-button compact" type="button" onClick={() => { setKeyword(''); setKeywordInput('') }}>重置</button></div>
          <div className="management-table-wrap"><table className="management-table"><thead><tr><th><input type="checkbox" aria-label="全选" /></th><th>标签名称</th><th>标签编码</th><th>标签颜色</th><th>适用层级</th><th>状态</th><th>创建时间</th><th>操作</th></tr></thead><tbody>{selected.tags.filter((item) => !keyword || item.name.includes(keyword)).map((item) => <tr key={item.id}><td><input type="checkbox" aria-label={`选择${item.name}`} /></td><td><strong>{item.name}</strong></td><td><code>{item.code}</code></td><td><span className="color-value"><i style={{ background: item.color }} />{item.color}</span></td><td><span className="scope-tag">{item.appliesTo === 'goal' ? '单次任务' : item.appliesTo === 'action' ? '小目标' : '历史兼容'}</span></td><td><span className={`enabled-tag ${item.enabled ? '' : 'disabled'}`}>{item.enabled ? '启用' : '停用'}</span></td><td>{item.createdAt}</td><td><div className="row-actions"><button type="button" onClick={() => openLabel(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => removeLabel(item)}><Trash2 size={15} />删除</button></div></td></tr>)}{!selected.tags.length && <tr><td colSpan={8}><div className="management-empty"><Tags size={34} />暂无标签</div></td></tr>}</tbody></table></div><footer className="management-footer"><span>共 {selected.tags.length} 条</span><span>标签仅在项目显式绑定后进入任务快照</span></footer>
        </> : <>
          <header className="management-toolbar"><div><h2>标签库列表</h2><p>维护系统级共享标签及颜色</p></div><button className="primary-button" type="button" onClick={() => openLibrary()}><Plus size={16} />创建标签库</button></header>
          ...
          <div className="management-table-wrap"><table className="management-table library-table"><thead><tr><th>标签库名称</th><th>标签库编码</th><th>描述</th><th>标签数量</th><th>创建时间</th><th>操作</th></tr></thead><tbody>{loading ? <tr><td colSpan={6}><div className="management-empty">正在加载标签库...</div></td></tr> : filteredLibraries.map((item) => <tr key={item.id}><td><div className="entity-name"><strong>{item.name}</strong><small>{item.enabled ? '系统共享' : '已停用'}</small></div></td><td><code>{item.code}</code></td><td>{item.desc || '-'}</td><td><strong className="count-link">{item.count}</strong></td><td>{item.createdAt}</td><td><div className="row-actions"><button type="button" onClick={() => setSelectedId(item.id)}><Eye size={15} />查看</button><button type="button" onClick={() => openLibrary(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => removeLibrary(item)}><Trash2 size={15} />删除</button><button className="icon-button small" type="button"><MoreHorizontal size={17} /></button></div></td></tr>)}{!loading && !filteredLibraries.length && <tr><td colSpan={6}><div className="management-empty"><CircleAlert size={32} />未找到匹配标签库</div></td></tr>}</tbody></table></div><footer className="management-footer"><span>共 {filteredLibraries.length} 条</span><div className="pagination"><button disabled>‹</button><strong>1</strong><button disabled>›</button><span>10条/页</span></div></footer>
        </>}
      </section>
    </section>
