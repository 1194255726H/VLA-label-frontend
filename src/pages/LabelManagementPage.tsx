...
export function LabelManagementPage({ session }: { session: SessionResponse }) {
  ...
  const allVisibleTagsSelected = Boolean(visibleTags.length) && visibleTags.every((item) => selectedLabelIds.has(item.id))
  ...
  async function removeLabel(item: LabelItem) { if (!selected || !window.confirm(`确认删除标签“${item.name}”？`)) return; try { setLibraries(await labelApi.deleteLabel(selected.id, item.id)); setSelectedLabelIds((current) => { const next = new Set(current); next.delete(item.id); return next }); setToast('标签已删除') } catch (reason) { setToast(reason instanceof Error ? reason.message : '删除失败') } }
  ...
  function toggleVisibleTags() {
    setSelectedLabelIds((current) => {
      ...
      visibleTags.forEach((item) => allVisibleTagsSelected ? next.delete(item.id) : next.add(item.id))
      return next
    })
  }
  async function exportLabels() {
    ...
    try { await labelApi.exportLabels(selected.id, [...selectedLabelIds]); setToast(selectedLabelIds.size ? `已导出 ${selectedLabelIds.size} 个标签` : '已导出全部标签') }
    catch (reason) { setToast(reason instanceof Error ? reason.message : '标签导出失败') }
    finally { setBatchAction(undefined) }
  }
  async function batchDeleteLabels() {
    if (!selected || !selectedLabelIds.size || !window.confirm(`确认批量删除已选的 ${selectedLabelIds.size} 个标签？此操作不可撤销。`)) return
    setBatchAction('delete')
    try { const result = await labelApi.batchDeleteLabels(selected.id, [...selectedLabelIds]); setLibraries(result.libraries); setSelectedLabelIds(new Set()); setPage(1); setToast(`已删除 ${result.deletedCount} 个标签`) }
    catch (reason) { setToast(reason instanceof Error ? reason.message : '标签批量删除失败') }
    finally { setBatchAction(undefined) }
  }
  ...
  return <AppShell user={session.account}>
    <section className="management-page">
      <section className="management-panel panel">
        {selected ? <>
          <header className="management-toolbar detail-toolbar"><div className="detail-title"><button className="icon-button bordered" type="button" onClick={() => { setSelectedId(undefined); setSelectedLabelIds(new Set()); setPage(1) }}><ArrowLeft size={17} /></button><div><h2 title={selected.name}>{selected.name}</h2><p title={`${selected.desc || '暂无标签库描述'} · ${selected.count} 个启用标签`}>{selected.desc || '暂无标签库描述'} · {selected.count} 个启用标签</p></div></div><div><button className="secondary-button" type="button" onClick={() => void downloadTemplate()}><Download size={16} />下载模板</button><button className="secondary-button" type="button" disabled={Boolean(batchAction)} onClick={() => void exportLabels()}><Download size={16} />{batchAction === 'export' ? '正在导出...' : '批量导出'}</button>{canWrite && <><button className="secondary-button batch-delete-button" type="button" disabled={!selectedLabelIds.size || Boolean(batchAction)} onClick={() => void batchDeleteLabels()}><Trash2 size={16} />{batchAction === 'delete' ? '正在删除...' : `批量删除${selectedLabelIds.size ? ` (${selectedLabelIds.size})` : ''}`}</button><button className="secondary-button" type="button" onClick={() => setImportModal(true)}><Upload size={16} />批量导入</button><button className="primary-button" type="button" onClick={() => openLabel()}><Plus size={16} />创建标签</button></>}</div></header>
          ...
          <div className="management-table-wrap"><table className="management-table batch-select-table"><thead><tr><th><input type="checkbox" aria-label="全选当前页标签" checked={allVisibleTagsSelected} onChange={toggleVisibleTags} /></th><th>标签名称</th><th>标签编码</th><th>标签颜色</th><th>适用层级</th><th>状态</th><th>创建时间</th><th>操作</th></tr></thead><tbody>{visibleTags.map((item) => <tr className={selectedLabelIds.has(item.id) ? 'selected-row' : ''} key={item.id}><td><input type="checkbox" aria-label={`选择${item.name}`} checked={selectedLabelIds.has(item.id)} onChange={() => setSelectedLabelIds((current) => { const next = new Set(current); if (next.has(item.id)) next.delete(item.id); else next.add(item.id); return next })} /></td><td><strong>{item.name}</strong></td><td><code>{item.code}</code></td><td><span className="color-value"><i style={{ background: item.color }} />{item.color}</span></td><td><span className="scope-tag">{item.appliesTo === 'goal' ? '单次任务' : item.appliesTo === 'action' ? '小目标' : '历史兼容'}</span></td><td><span className={`enabled-tag ${item.enabled ? '' : 'disabled'}`}>{item.enabled ? '启用' : '停用'}</span></td><td>{formatDateTime(item.createdAt)}</td><td><div className="row-actions">{canWrite && <><button type="button" onClick={() => openLabel(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => removeLabel(item)}><Trash2 size={15} />删除</button></>}</div></td></tr>)}{!filteredTags.length && <tr><td colSpan={8}><div className="management-empty"><Tags size={34} />暂无标签</div></td></tr>}</tbody></table></div><footer className="management-footer"><span>共 {filteredTags.length} 条{selectedLabelIds.size ? ` · 已选 ${selectedLabelIds.size} 条` : ''}</span><PaginationJump page={page} pages={pages} onChange={setPage} pageSize={pageSize} onPageSizeChange={(size) => { setPageSize(size); setPage(1) }} /></footer>
        </> : <>
