...
export function LabelManagementPage({ session }: { session: SessionResponse }) {
  ...
  const pages = Math.max(1, Math.ceil(activeItems.length / 10))
  const currentPage = Math.min(page, pages)
  const visibleLibraries = filteredLibraries.slice((currentPage - 1) * 10, currentPage * 10)
  const visibleTags = filteredTags.slice((currentPage - 1) * 10, currentPage * 10)
