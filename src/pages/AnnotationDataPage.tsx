...
export function AnnotationDataPage({ session }: { session: SessionResponse }) {
  ...
  const [fleetOpen, setFleetOpen] = useState(false); const [toast, setToast] = useState('')
  ...
  async function fleetSynced(message: string) { setFleetOpen(false); setLoading(true); try { setItems(await annotationDataApi.list(projectId)); setToast(message) } catch { setToast(`${message}，但数据列表刷新失败`) } finally { setLoading(false) } }
  ...
  return <AppShell user={session.account}><section className="management-page"><section className="management-panel panel">
    ...
    <div className="annotation-data-tabs"><div className="status-segments">{statuses.map((item) => <button key={item.key} type="button" className={status === item.key ? 'active' : ''} onClick={() => setStatus(item.key)}>{item.label}<span>{counts[item.key] || 0}</span></button>)}</div><button className="primary-button" type="button" onClick={() => setFleetOpen(true)}><Database size={16} />从 Fleet 同步</button></div>
