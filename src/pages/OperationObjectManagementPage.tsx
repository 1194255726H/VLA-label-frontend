import { ArrowLeft, Edit3, Eye, PackageOpen, Plus, Search, Trash2 } from 'lucide-react'
...
export function OperationObjectManagementPage({ session }: { session: SessionResponse }) {
  ...
  const pendingCount = useMemo(() => objects.filter((item) => !item.approved).length, [objects])
  ...
  return <AppShell user={session.account}><section className="management-page"><section className="management-panel panel">
    <header className="management-toolbar detail-toolbar"><div className="detail-title">{selected && <button className="icon-button bordered" type="button" onClick={() => { setSelected(undefined); setKeyword(''); setKeywordInput(''); setPage(1) }}><ArrowLeft size={17} /></button>}<div><h2>{selected ? selected.name : '操作对象库列表'}</h2><p>{selected ? `${selected.desc || '暂无描述'} · 当前页 ${objects.length} 个对象 · ${pendingCount} 个待审核` : '集中维护关键帧可关联的操作对象'}</p></div></div>{canWrite && <button className="primary-button" type="button" onClick={() => selected ? openObject() : openLibrary()}><Plus size={16} />{selected ? '创建操作对象' : '创建对象库'}</button>}</header>
    ...
    <div className="management-table-wrap"><table className="management-table library-table"><thead>{selected ? <tr><th>ID</th><th>对象名称</th><th>别名</th><th>属性</th><th>审核状态</th><th>创建时间</th><th>操作</th></tr> : <tr><th>对象库名称</th><th>描述</th><th>创建时间</th><th>操作</th></tr>}</thead><tbody>
      {loading ? <tr><td colSpan={selected ? 7 : 4}><div className="management-empty">正在加载...</div></td></tr> : selected ? objects.map((item) => <tr key={item.id}><td><code>{item.id}</code></td><td><strong>{item.name}</strong></td><td>{item.alias || '-'}</td><td>{item.attribute || '-'}</td><td><span className={`enabled-tag${item.approved ? '' : ' disabled'}`}>{item.approved ? '已通过' : '待审核'}</span></td><td>{formatDateTime(item.createdAt)}</td><td><div className="row-actions">{canWrite && <><button type="button" onClick={() => openObject(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => void removeObject(item)}><Trash2 size={15} />删除</button></>}</div></td></tr>) : libraries.map((item) => <tr key={item.id}><td><strong className="count-link">{item.name}</strong></td><td>{item.desc || '-'}</td><td>{formatDateTime(item.createdAt)}</td><td><div className="row-actions"><button type="button" onClick={() => { setSelected(item); setKeyword(''); setKeywordInput(''); setPage(1) }}><Eye size={15} />查看</button>{canWrite && <><button type="button" onClick={() => openLibrary(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => void removeLibrary(item)}><Trash2 size={15} />删除</button></>}</div></td></tr>)}
      {!loading && !(selected ? objects : libraries).length && <tr><td colSpan={selected ? 7 : 4}><div className="management-empty"><PackageOpen size={34} />暂无数据</div></td></tr>}
    </tbody></table></div><footer className="management-footer"><span>共 {total} 条</span><div className="pagination"><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>‹</button><strong>{page}</strong><button type="button" disabled={page >= pages} onClick={() => setPage((value) => value + 1)}>›</button><span>10条/页</span></div></footer>
  </section></section>
