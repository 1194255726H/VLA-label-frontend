...
export function OperationObjectManagementPage({ session }: { session: SessionResponse }) {
  ...
  return <AppShell user={session.account}><section className="management-page"><section className="management-panel panel">
    <header className="management-toolbar detail-toolbar"><div className="detail-title">{selected && <button className="icon-button bordered" type="button" onClick={() => { setSelected(undefined); setSelectedObjectIds(new Set()); setApprovalTab('all'); setKeyword(''); setKeywordInput(''); setPage(1) }}><ArrowLeft size={17} /></button>}<div><h2>{selected ? selected.name : '操作对象库列表'}</h2><p>{selected ? `${selected.desc || '暂无描述'} · 共 ${selected.objectCount} 个对象 · ${selected.pendingApprovalCount} 个待审核` : '集中维护关键帧可关联的操作对象'}</p></div></div>{canWrite && <div>{selected && <><button className="secondary-button" type="button" onClick={() => void downloadTemplate()}><Download size={16} />下载模板</button><button className="secondary-button" type="button" disabled={Boolean(batchAction)} onClick={() => void exportObjects()}><Download size={16} />{batchAction === 'export' ? '正在导出...' : '批量导出'}</button><button className="secondary-button batch-delete-button" type="button" disabled={!selectedObjectIds.size || Boolean(batchAction)} onClick={() => void batchDeleteObjects()}><Trash2 size={16} />{batchAction === 'delete' ? '正在删除...' : `批量删除${selectedObjectIds.size ? ` (${selectedObjectIds.size})` : ''}`}</button><button className="secondary-button" type="button" onClick={() => setImportModal(true)}><Upload size={16} />批量导入</button></>}<button className="primary-button" type="button" onClick={() => selected ? openObject() : openLibrary()}><Plus size={16} />{selected ? '创建操作对象' : '创建对象库'}</button></div>}</header>
