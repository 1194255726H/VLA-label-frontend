...
export function OperationObjectManagementPage({ session }: { session: SessionResponse }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      if (selected) { const result = await operationObjectApi.listObjects(selected.id, { keyword, page, pageSize }); setObjects(result.items); setTotal(result.total) }
  ...
  }, [keyword, page, pageSize, selected])
