...
export function OperationObjectManagementPage({ session }: { session: SessionResponse }) {
  ...
  const [batchAction, setBatchAction] = useState<'export' | 'delete'>()
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      if (selected) { const result = await operationObjectApi.listObjects(selected.id, { keyword, approved: approvalTab === 'all' ? undefined : approvalTab === 'approved', page, pageSize }); setObjects(result.items); setTotal(result.total) }
      else { const result = await operationObjectApi.listLibraries({ keyword, page, pageSize }); setLibraries(result.items); setTotal(result.total) }
    } catch (reason) { setToast(reason instanceof Error ? reason.message : '数据加载失败') }
    finally { setLoading(false) }
  }, [approvalTab, keyword, page, pageSize, selected])

  useEffect(() => { void Promise.resolve().then(load) }, [load])
  useEffect(() => { if (!toast) return; const timer = window.setTimeout(() => setToast(''), 2500); return () => clearTimeout(timer) }, [toast])
  const pages = Math.max(1, Math.ceil(total / pageSize))
  const allVisibleObjectsSelected = Boolean(objects.length) && objects.every((item) => selectedObjectIds.has(item.id))
  function search() { setSelectedObjectIds(new Set()); setPage(1); setKeyword(keywordInput.trim()) }
  ...
  async function removeObject(item: OperationObject) { if (!selected || !window.confirm(`确认删除操作对象“${item.name}”？已引用关键帧将解除关联。`)) return; try { await operationObjectApi.deleteObject(selected.id, item.id); setSelectedObjectIds((current) => { const next = new Set(current); next.delete(item.id); return next }); setToast('操作对象已删除'); await load() } catch (reason) { setToast(reason instanceof Error ? reason.message : '删除失败') } }
  ...
  function toggleVisibleObjects() {
    setSelectedObjectIds((current) => {
      const next = new Set(current)
      objects.forEach((item) => allVisibleObjectsSelected ? next.delete(item.id) : next.add(item.id))
      return next
    })
  }
  async function exportObjects() {
    ...
    try { await operationObjectApi.exportObjects(selected.id, [...selectedObjectIds]); setToast(selectedObjectIds.size ? `已导出 ${selectedObjectIds.size} 个操作对象` : '已导出全部操作对象') }
    catch (reason) { setToast(reason instanceof Error ? reason.message : '操作对象导出失败') }
    finally { setBatchAction(undefined) }
  }
  async function batchDeleteObjects() {
    if (!selected || !selectedObjectIds.size || !window.confirm(`确认批量删除已选的 ${selectedObjectIds.size} 个操作对象？已引用关键帧将解除关联，此操作不可撤销。`)) return
    setBatchAction('delete')
    try {
      const deletedCount = await operationObjectApi.batchDeleteObjects(selected.id, [...selectedObjectIds])
      setSelectedObjectIds(new Set())
      setPage(1)
      if (page === 1) await load()
      setToast(`已删除 ${deletedCount} 个操作对象`)
    } catch (reason) { setToast(reason instanceof Error ? reason.message : '操作对象批量删除失败') }
    finally { setBatchAction(undefined) }
  }

  return <AppShell user={session.account}><section className="management-page"><section className="management-panel panel">
    <header className="management-toolbar detail-toolbar"><div className="detail-title">{selected && <button className="icon-button bordered" type="button" onClick={() => { setSelected(undefined); setSelectedObjectIds(new Set()); setApprovalTab('all'); setKeyword(''); setKeywordInput(''); setPage(1) }}><ArrowLeft size={17} /></button>}<div><h2 title={selected ? selected.name : '操作对象库列表'}>{selected ? selected.name : '操作对象库列表'}</h2><p title={selected ? `${selected.desc || '暂无描述'} · 共 ${selected.objectCount} 个对象 · ${selected.pendingApprovalCount} 个待审核` : '集中维护关键帧可关联的操作对象'}>{selected ? `${selected.desc || '暂无描述'} · 共 ${selected.objectCount} 个对象 · ${selected.pendingApprovalCount} 个待审核` : '集中维护关键帧可关联的操作对象'}</p></div></div>{canWrite && <div>{selected && <><button className="secondary-button" type="button" onClick={() => void downloadTemplate()}><Download size={16} />下载模板</button><button className="secondary-button" type="button" disabled={Boolean(batchAction)} onClick={() => void exportObjects()}><Download size={16} />{batchAction === 'export' ? '正在导出...' : '批量导出'}</button><button className="secondary-button batch-delete-button" type="button" disabled={!selectedObjectIds.size || Boolean(batchAction)} onClick={() => void batchDeleteObjects()}><Trash2 size={16} />{batchAction === 'delete' ? '正在删除...' : `批量删除${selectedObjectIds.size ? ` (${selectedObjectIds.size})` : ''}`}</button><button className="secondary-button" type="button" onClick={() => setImportModal(true)}><Upload size={16} />批量导入</button></>}<button className="primary-button" type="button" onClick={() => selected ? openObject() : openLibrary()}><Plus size={16} />{selected ? '创建操作对象' : '创建对象库'}</button></div>}</header>
    {selected && <div className="operation-object-tabs" role="tablist" aria-label="操作对象审核状态"><button type="button" role="tab" aria-selected={approvalTab === 'all'} className={approvalTab === 'all' ? 'active' : ''} onClick={() => { setSelectedObjectIds(new Set()); setApprovalTab('all'); setPage(1) }}>全部</button><button type="button" role="tab" aria-selected={approvalTab === 'approved'} className={approvalTab === 'approved' ? 'active' : ''} onClick={() => { setSelectedObjectIds(new Set()); setApprovalTab('approved'); setPage(1) }}>已审核</button><button type="button" role="tab" aria-selected={approvalTab === 'pending'} className={approvalTab === 'pending' ? 'active' : ''} onClick={() => { setSelectedObjectIds(new Set()); setApprovalTab('pending'); setPage(1) }}>待审核</button></div>}
    <div className="management-filters"><label><span>{selected ? '操作对象' : '对象库名称'}</span><div className="filter-control"><Search size={16} /><input value={keywordInput} onChange={(event) => setKeywordInput(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && search()} placeholder={selected ? '请输入对象名称或 ID' : '请输入对象库名称或描述'} /></div></label><button className="primary-button compact" type="button" onClick={search}>查询</button><button className="secondary-button compact" type="button" onClick={() => { setSelectedObjectIds(new Set()); setKeywordInput(''); setKeyword(''); setPage(1) }}>重置</button></div>
    <div className="management-table-wrap"><table className={`management-table library-table${selected ? ' batch-select-table' : ''}`}><thead>{selected ? <tr><th><input type="checkbox" aria-label="全选当前页操作对象" checked={allVisibleObjectsSelected} onChange={toggleVisibleObjects} /></th><th>ID</th><th>对象名称</th><th>别名</th><th>属性</th><th>审核状态</th><th>创建时间</th><th>操作</th></tr> : <tr><th>对象库名称</th><th>描述</th><th>对象总数</th><th>待审核数</th><th>创建时间</th><th>操作</th></tr>}</thead><tbody>
      {loading ? <tr><td colSpan={selected ? 8 : 6}><div className="management-empty">正在加载...</div></td></tr> : selected ? objects.map((item) => <tr className={selectedObjectIds.has(item.id) ? 'selected-row' : ''} key={item.id}><td><input type="checkbox" aria-label={`选择${item.name}`} checked={selectedObjectIds.has(item.id)} onChange={() => setSelectedObjectIds((current) => { const next = new Set(current); if (next.has(item.id)) next.delete(item.id); else next.add(item.id); return next })} /></td><td><code>{item.id}</code></td><td><strong>{item.name}</strong></td><td>{item.alias || '-'}</td><td>{item.attribute || '-'}</td><td><span className={`enabled-tag${item.approved ? '' : ' disabled'}`}>{item.approved ? '已通过' : '待审核'}</span></td><td>{formatDateTime(item.createdAt)}</td><td><div className="row-actions">{canWrite && <><button type="button" onClick={() => openObject(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => void removeObject(item)}><Trash2 size={15} />删除</button></>}</div></td></tr>) : libraries.map((item) => <tr key={item.id}><td><strong className="count-link">{item.name}</strong></td><td>{item.desc || '-'}</td><td><strong>{item.objectCount ?? 0}</strong></td><td><span className={`enabled-tag${item.pendingApprovalCount ? ' disabled' : ''}`}>{item.pendingApprovalCount ?? 0}</span></td><td>{formatDateTime(item.createdAt)}</td><td><div className="row-actions"><button type="button" onClick={() => { setSelected(item); setSelectedObjectIds(new Set()); setApprovalTab('all'); setKeyword(''); setKeywordInput(''); setPage(1) }}><Eye size={15} />查看</button>{canWrite && <><button type="button" onClick={() => openLibrary(item)}><Edit3 size={15} />编辑</button><button className="danger-action" type="button" onClick={() => void removeLibrary(item)}><Trash2 size={15} />删除</button></>}</div></td></tr>)}
      {!loading && !(selected ? objects : libraries).length && <tr><td colSpan={selected ? 8 : 6}><div className="management-empty"><PackageOpen size={34} />暂无数据</div></td></tr>}
    </tbody></table></div><footer className="management-footer"><span>共 {total} 条{selectedObjectIds.size ? ` · 已选 ${selectedObjectIds.size} 条` : ''}</span><PaginationJump page={page} pages={pages} disabled={loading} onChange={setPage} pageSize={pageSize} onPageSizeChange={(size) => { setPageSize(size); setPage(1) }} /></footer>
  </section></section>
  {libraryModal && <Modal title={editingLibrary ? '编辑对象库' : '创建对象库'} onClose={() => setLibraryModal(false)} footer={<><button className="secondary-button" type="button" onClick={() => setLibraryModal(false)}>取消</button><button className="primary-button" type="submit" form="operation-library-form">保存</button></>}><form id="operation-library-form" className="single-column-form" onSubmit={saveLibrary}><label><span>对象库名称 <i className="required-mark">*</i></span><input value={libraryForm.name} maxLength={100} onChange={(event) => setLibraryForm({ ...libraryForm, name: event.target.value })} /></label><label><span>描述</span><textarea value={libraryForm.desc} maxLength={200} onChange={(event) => setLibraryForm({ ...libraryForm, desc: event.target.value })} /></label>{error && <p className="inline-error">{error}</p>}</form></Modal>}
