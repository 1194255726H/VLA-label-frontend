...
export function OperationObjectManagementPage({ session }: { session: SessionResponse }) {
  ...
  return <AppShell user={session.account}><section className="management-page"><section className="management-panel panel">
    <header className="management-toolbar detail-toolbar"><div className="detail-title">{selected && <button className="icon-button bordered" type="button" onClick={() => { setSelected(undefined); setApprovalTab('all'); setKeyword(''); setKeywordInput(''); setPage(1) }}><ArrowLeft size={17} /></button>}<div><h2>{selected ? selected.name : '操作对象库列表'}</h2><p>{selected ? `${selected.desc || '暂无描述'} · 共 ${selected.objectCount} 个对象 · ${selected.pendingApprovalCount} 个待审核` : '集中维护关键帧可关联的操作对象'}</p></div></div>{canWrite && <div>{selected && <><button className="secondary-button" type="button" onClick={() => void downloadTemplate()}><Download size={16} />下载模板</button><button className="secondary-button" type="button" onClick={() => setImportModal(true)}><Upload size={16} />批量导入</button></>}<button className="primary-button" type="button" onClick={() => selected ? openObject() : openLibrary()}><Plus size={16} />{selected ? '创建操作对象' : '创建对象库'}</button></div>}</header>
    {selected && <div className="operation-object-tabs" role="tablist" aria-label="操作对象审核状态"><button type="button" role="tab" aria-selected={approvalTab === 'all'} className={approvalTab === 'all' ? 'active' : ''} onClick={() => { setApprovalTab('all'); setPage(1) }}>全部</button><button type="button" role="tab" aria-selected={approvalTab === 'approved'} className={approvalTab === 'approved' ? 'active' : ''} onClick={() => { setApprovalTab('approved'); setPage(1) }}>已审核</button><button type="button" role="tab" aria-selected={approvalTab === 'pending'} className={approvalTab === 'pending' ? 'active' : ''} onClick={() => { setApprovalTab('pending'); setPage(1) }}>待审核</button></div>}
    ...
    </tbody></table></div><footer className="management-footer"><span>共 {total} 条</span><PaginationJump page={page} pages={pages} disabled={loading} onChange={setPage} pageSize={pageSize} onPageSizeChange={(size) => { setPageSize(size); setPage(1) }} /></footer>
  </section></section>
