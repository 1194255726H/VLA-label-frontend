  ...
  return <AppShell user={session.account}>
    <section className="management-page team-page">
      <aside className="team-tree panel">
        ...
        <nav><button className={selectedTeam === 'all' ? 'active' : ''} type="button" onClick={() => setSelectedTeam('all')}><span><UsersRound size={17} />全部团队</span><b>{data.members.length}</b></button>{data.teams.filter((item) => item.name.includes(teamSearch)).map((item) => <button className={selectedTeam === item.id ? 'active' : ''} type="button" key={item.id} onClick={() => setSelectedTeam(item.id)}><span><UsersRound size={17} />{item.name}</span><b>{data.members.filter((member) => member.team === item.name).length}</b><ChevronRight size={14} /></button>)}</nav>
      ...
      <div className="team-main">
        ...
        <section className="management-panel panel team-workspace">
          ...
          {tab === 'members' ? <>
            <div className="management-filters"><label><span>成员</span><div className="filter-control"><Search size={16} /><input value={keywordInput} onChange={(e) => setKeywordInput(e.target.value)} placeholder="姓名、账号或邮箱" /></div></label><label><span>角色</span><div className="filter-control select"><select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}><option value="">全部角色</option>{roles.map((role) => <option key={role}>{role}</option>)}</select></div></label><label><span>状态</span><div className="filter-control select"><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">全部状态</option><option value="true">启用</option><option value="false">停用</option></select></div></label><button className="primary-button compact" type="button" onClick={() => setKeyword(keywordInput.trim())}>查询</button><button className="secondary-button compact" onClick={() => { setKeyword(''); setKeywordInput(''); setRoleFilter(''); setStatusFilter('') }}>重置</button></div>
            <div className="management-table-wrap"><table className="management-table member-table"><thead><tr><th>成员</th><th>团队</th><th>角色</th><th>账号</th><th>邮箱</th><th>项目数</th><th>状态</th><th>添加日期</th><th>操作</th></tr></thead><tbody>{loading ? <tr><td colSpan={9}><div className="management-empty">正在加载成员...</div></td></tr> : visibleMembers.map((item) => <tr key={item.accountId}><td><div className="member-identity"><span>{item.name.slice(-2)}</span><strong>{item.name}</strong></div></td><td>{item.team || '未分配团队'}</td><td><div className="role-list">{item.roles.map((role) => <span key={role}>{role}</span>)}</div></td><td>{item.account}</td><td>{item.email || '-'}</td><td>{item.projects.length}</td><td><button className={`status-switch ${item.enabled ? 'on' : ''}`} type="button" onClick={() => toggleMember(item)} aria-label={item.enabled ? '停用成员' : '启用成员'}><i /></button><span className="switch-label">{item.enabled ? '启用' : '停用'}</span></td><td>{item.joinedAt}</td><td><div className="row-actions"><button type="button" onClick={() => openMember(item)}><Edit3 size={15} />编辑</button><button type="button" onClick={() => openChangePassword(item)}><KeyRound size={15} />修改密码</button><button className="danger-action" type="button" onClick={() => removeMember(item)}><Trash2 size={15} />删除</button></div></td></tr>)}{!loading && !visibleMembers.length && <tr><td colSpan={9}><div className="management-empty"><CircleAlert size={32} />暂无符合条件的成员</div></td></tr>}</tbody></table></div><footer className="management-footer"><span>共 {visibleMembers.length} 条</span><div className="pagination"><button disabled>‹</button><strong>1</strong><button disabled>›</button><span>10条/页</span></div></footer>
