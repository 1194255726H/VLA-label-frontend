...
export function TeamMembersPage({ session }: { session: SessionResponse }) {
  ...
  const memberPages = Math.max(1, Math.ceil(visibleMembers.length / 10))
  const currentMemberPage = Math.min(page, memberPages)
  const pagedMembers = visibleMembers.slice((currentMemberPage - 1) * 10, currentMemberPage * 10)
