...
import { nodeToneByLabel } from '../utils/node'
...
export function ProjectManagementPage({ session }: { session: SessionResponse }) {
  ...
  function openCreate() { setEditingId(undefined); setEditingReadOnly(false); setForm(emptyForm); setLabelKeyword(''); setLabelPickerOpen(false); setUploadingGuideline(false); setStep(1); setError(''); setModalOpen(true); void loadLabelLibraryOptions() }
  async function openEdit(item: ManagedProject) {
    if (item.status === 'archived') return setToast('已归档项目不可编辑')
    setEditingReadOnly(!['not-started', 'paused'].includes(item.status))
  ...
  return <AppShell user={session.account}>
    <section className="management-page">
      <section className="management-panel panel">
        <header className="management-toolbar">
          <div className="status-segments"><button className={status === 'all' ? 'active' : ''} onClick={() => { setStatus('all'); setPage(1) }} type="button">全部 <span>{items.length}</span></button>{Object.entries(statusLabels).map(([key, label]) => <button className={status === key ? 'active' : ''} onClick={() => { setStatus(key as ProjectStatus); setPage(1) }} type="button" key={key}>{label} <span>{counts[key] || 0}</span></button>)}</div>
        ...
        <div className="management-filters">
          ...
          <label><span>所属团队</span><div className="filter-control select"><select value={team} onChange={(event) => { setTeam(event.target.value); setPage(1) }}><option value="">全部团队</option>{teams.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select><ChevronDown size={14} /></div></label>
          <button className="primary-button compact" type="button" onClick={() => { setKeyword(keywordInput.trim()); setPage(1) }}>查询</button><button className="secondary-button compact" type="button" onClick={() => { setKeywordInput(''); setKeyword(''); setTeam(''); setPage(1) }}>重置</button>
        </div>
        <div className="management-table-wrap"><table className="management-table project-table"><thead><tr><th>项目名称</th><th>状态</th><th>团队</th><th>标注人数</th><th>数据量</th><th>视频时长</th><th>有效时长</th><th>无效时长</th><th>单次任务数</th><th>小目标数</th><th>完成节点</th><th>进度</th><th>负责人</th><th>交付时间</th><th>操作</th></tr></thead><tbody>
          {loading ? <tr><td colSpan={15}><div className="management-empty">正在加载项目...</div></td></tr> : visibleProjects.map((item) => <tr key={item.id}><td><div className="entity-name"><strong>{item.name}</strong><small>{item.code}</small></div></td><td><span className={`project-status ${item.status}`}>{statusLabels[item.status]}</span></td><td title={item.teams.join('、')}>{item.teams.join('、')}</td><td>{item.memberCount}</td><td>{item.dataCount}</td><td>{duration(item.duration || 0)}</td><td>{duration(item.validDuration)}</td><td>{duration(item.invalidDuration)}</td><td>{item.goalCount}</td><td>{item.actionCount}</td><td><span className={`node-tag ${nodeToneByLabel(item.completionNode)}`}>{item.completionNode}</span></td><td><div className="progress-cell"><span><i style={{ width: `${item.progress}%` }} /></span><b>{item.progress}%</b></div></td><td>{item.owner}</td><td><span className={new Date(item.deliveryAt) < new Date() && item.status !== 'finished' ? 'risk-date' : ''}>{item.deliveryAt || '-'}</span></td><td><div className="row-actions"><button type="button" onClick={() => { window.location.hash = `/projects/${encodeURIComponent(item.id)}/annotation-data` }}><Eye size={15} />查看</button><button type="button" disabled={item.status === 'archived' || Boolean(openingProjectId)} title={item.status === 'archived' ? '已归档项目不可编辑' : item.status === 'running' ? '进行中项目仅可查看，暂停后可编辑' : item.status === 'finished' ? '已结束项目仅可查看' : undefined} onClick={() => void openEdit(item)}><Edit3 size={15} />{openingProjectId === item.id ? '加载中' : '编辑'}</button>{statusActions[item.status].map(({ label, status: next, icon: Icon }) => <button key={label} type="button" onClick={() => changeStatus(item, next)}><Icon size={15} />{label}</button>)}<button className="danger-action" type="button" onClick={() => removeProject(item)}><Trash2 size={15} />删除</button></div></td></tr>)}
    ...
    {modalOpen && <Modal title={editingReadOnly ? '查看项目' : editingId ? '编辑项目' : '创建项目'} onClose={() => setModalOpen(false)} footer={editingReadOnly ? <><button className="secondary-button" type="button" onClick={() => step === 2 ? setStep(1) : setModalOpen(false)}>{step === 2 ? '上一步' : '关闭'}</button><button className="primary-button" type="button" onClick={() => step === 1 ? setStep(2) : setModalOpen(false)}>{step === 1 ? '下一步' : '关闭'}</button></> : <><button className="secondary-button" type="button" disabled={uploadingGuideline} onClick={() => step === 2 ? setStep(1) : setModalOpen(false)}>{step === 2 ? '上一步' : '取消'}</button><button className="primary-button" type="submit" form="project-form" disabled={uploadingGuideline}>{uploadingGuideline ? '文件上传中...' : step === 1 ? '下一步' : editingId ? '保存' : '创建项目'}</button></>}>
